...
int main() {
	...
	// TEXTURE
	Texture wall((parentDir + texPath + "container.jpg").c_str(), GL_TEXTURE_2D, GL_TEXTURE0, GL_RGB, GL_UNSIGNED_BYTE, GL_REPEAT, GL_NEAREST);
	Texture face((parentDir + texPath + "awesomeface.png").c_str(), GL_TEXTURE_2D, GL_TEXTURE1, GL_RGBA, GL_UNSIGNED_BYTE, GL_REPEAT, GL_NEAREST);
	
	wall.texUnit(shaderProgram, "texture1", 0);
	face.texUnit(shaderProgram, "texture2", 1);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		shaderProgram.SetFloat("opacity", opacityValue);
...
void processInput(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS && opacityValue < 1) {
		opacityValue += 0.0001f;
	}

	if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS && opacityValue > 0) {
		opacityValue -= 0.0001f;
	}
}