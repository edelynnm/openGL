...
int main() {
	...
	// TEXTURE
	Texture obj((parentDir + texPath + "psse-logo.jpg").c_str(), GL_TEXTURE_2D, GL_TEXTURE0, GL_RGB, GL_UNSIGNED_BYTE, GL_REPEAT, GL_NEAREST);

	obj.texUnit(shaderProgram, "texture1", 0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Draw triangle
		obj.Bind(GL_TEXTURE0);

		shaderProgram.Use();

		glm::mat4 transform = glm::mat4(1.0f);
		float translateValue = (pow(sin(glfwGetTime()), 2));
		float scaleValue = 1 - (pow(sin(glfwGetTime()), 2));
		float rotationAngle = - (pow(sin(glfwGetTime()), 2));

		transform = glm::translate(transform, glm::vec3(0.0f, -translateValue, 0.0f));
